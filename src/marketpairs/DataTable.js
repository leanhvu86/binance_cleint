...
const Row = (props) => (
    <React.Fragment>
        <div className="d-none d-sm-inline" >
            <Link className="row table-row small py-1" to={`trade/${props.symbol}`} style={styleTable}>
                <div className="col">{props.symbol}</div>
                <div className={props.priceChangePercent < 0 ? 'col text-danger' : 'col text-success'}>{new BigNumber(props.lastPrice).toFormat(null,1)}</div>
